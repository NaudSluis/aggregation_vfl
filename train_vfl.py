    ...
    for epoch in range(epoch_number, EPOCHS):
        ...
        with torch.no_grad():
            for i, vdata in pbar_val:
                ...
                if epoch == 0 and i == 0:
                    print("[DEBUG] Scaling check (VFL):")
                    print(f"  bldg_mean[0]: {bldg_mean[0].item():.4f}, bldg_std[0]: {bldg_std[0].item():.4f}")
                    print(f"  vy_future (scaled) stats: min={vy_future.min().item():.4f}, "
                        f"max={vy_future.max().item():.4f}, mean={vy_future.mean().item():.4f}")
                    print(f"  true_vy_future (original) stats: min={true_vy_future.min().item():.4f}, "
                        f"max={true_vy_future.max().item():.4f}, mean={true_vy_future.mean().item():.4f}")
